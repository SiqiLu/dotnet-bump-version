...
const commit = (filesToCommit, eventContext, message, githubToken) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield (0, exec_1.exec)("git", ["config", "user.name", `"${eventContext.pusher.name}"`], options);
        core.info(`git config user.email "${eventContext.pusher.email}"`);
        yield (0, exec_1.exec)("git", ["config", "user.email", `"${eventContext.pusher.email}"`], options);
        // Issue #10
        ...
        yield (0, exec_1.exec)("git", ["checkout", eventContext.branch], options);
        ...
        yield (0, exec_1.exec)("git", ["pull", "--no-edit", "--commit", "--strategy-option", "theirs", "publisher", eventContext.branch], options);
        core.info(`git push publisher ${eventContext.branch}`);
        yield (0, exec_1.exec)("git", ["push", "publisher", eventContext.branch], options);
    }
    catch (error) {
        if (error instanceof Error) {
