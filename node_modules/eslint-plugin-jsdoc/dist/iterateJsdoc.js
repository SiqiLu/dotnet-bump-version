...
const getUtils = (node, jsdoc, jsdocNode, settings, report, context, iteratingAll, ruleConfig) => {
  ...
  const hasSchemaOption = prop => {
    ...
    return _lodash.default.get(context, `options[0].${prop}`, schemaProperties[prop] && schemaProperties[prop].default);
  ...
  utils.avoidDocs = () => {
    ...
    if (!hasSchemaOption('checkConstructors') && (utils.isConstructor() || utils.hasATag(['class', 'constructor'])) || !hasSchemaOption('checkGetters') && utils.isGetter() || !hasSchemaOption('checkSetters') && utils.isSetter()) {
      return true;
    }

    const exemptedBy = _lodash.default.get(context, 'options[0].exemptedBy', ['inheritDoc', ...(settings.mode === 'closure' ? [] : ['inheritdoc'])]);
